<div class="row">
    ...
    <div class="input-field col s12 m6">
        ...
        <input class="validate" id="email" name="email" value="{{ old('email',isset($data) ? $data->email : null ) }}" type="text">
    </div>
    <div class="input-field col s12 m6">
        <label for="password"> Password @if ($errors->has('password')) <small class="red-text"> [ {{ $errors->first('password') }} ] </small> @endif</label>
        <input class="validate" id="password" name="password" value="{{ old('password',isset($data) ? $data->password : null ) }}" type="password">
    </div>
